...
/* Application set-up								*/
    ...
    Task = TSKcreate("App DMA", 2, 8192, &Task_DMA, 0);
    TSKsetCore(Task, 1);
    TSKresume(Task);

    Task = TSKcreate("App SPI", 2, 8192, &Task_SPI, 0);
    TSKsetCore(Task, 1);
    TSKresume(Task);
    
    Task = TSKcreate("App Display File", 0, 8192, &Task_DisplayFile, 0);
    TSKsetCore(Task, 1);							/* Create new task, will always run on core #1	*/
    TSKresume(Task);
    
    Task = TSKcreate("App State", 2, 8192, &Task_State, 0);
    TSKsetCore(Task, 0);
    TSKresume(Task);
